...
def get_tests():
    ...
    max_error = len(tests_)
    return tests_, test, observation, max_error
...
#from neuronunit.models.NeuroML2 import model_parameters as modelp
from neuronunit.optimization.optimization_management import nunit_evaluation, update_deap_pop, build_grid_wrapper
...
# build_grid_wrapper(means,stds=None,boundaries=None,k=5)    
def pre_run(tests,opt_keys):
    ...
    vp = modelp.vanilla
    npoints = 2**len(opt_keys)
    # from neuronunit.optimization.optimization_management import nunit_evaluation, update_deap_pop, build_grid_wrapper
    nparams = len(opt_keys)
    bd = {}

    # The first grid should simply be centered around vanilla.
    # values of Izhikititch
    bd['b'] = [ vp['b']-10*np.abs(vp['b']) , vp['b']+10*np.abs(vp['b']) ]
    bd['vr'] = [ vp['vr']-10*np.abs(vp['vr']) , vp['vr']+10*np.abs(vp['vr']) ]
    bd['a'] = [ vp['a']-10*np.abs(vp['a']) , vp['a']+10*np.abs(vp['a']) ]
    ...
    for key in opt_keys:
        ...
        while range_adj == True:
            ...
            # the problem is that new is not a list
